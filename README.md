...
2. **角速度推定** -- OpenCV.js の Farneback 光学フローで各フレーム間のピクセル移動を計算し、グリッド点ごとに角速度 `ω ≈ (flow・t) / r` を求め、トリム平均（上下10%外れ値除去）で集約
3. **角度算出** -- 角速度を積分して累積角度 θ を求め、最終フレームが 2π（1周）になるよう正規化
4. **均等サンプリング** -- 目標角度 `0, 2π/N, 2×2π/N, ...` に最も近いフレームを選択。角速度スパイク（隣接フレーム比1.5倍超）のフレームはモーションブラー回避のため1フレームずらして出力
